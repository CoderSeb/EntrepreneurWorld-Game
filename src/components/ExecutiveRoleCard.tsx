...
export function ExecutiveRoleCard({ role, onHire, canAfford = true, nowUnix }: ExecutiveRoleCardProps) {
  const { formatMoneyCompact } = useGameFormat();
  const { t } = useTranslation();

  const contractHoursRemaining =
    role.hired && role.expiresAtUnix != null
      ? Math.max(0, Math.ceil((role.expiresAtUnix - (nowUnix ?? Math.floor(Date.now() / 1000))) / 3600))
      : null;
    ...
    <View style={[styles.card, role.hired && styles.cardHired]}>
      ...
      <Text style={styles.salary}>
        {interpolate(t.executive.salaryLine, {
          ...
          hours: role.contractHours,
        })}
      </Text>
      {contractHoursRemaining !== null ? (
        <Text style={styles.contractMeta}>
          {interpolate(t.executive.contractExpires, { hours: contractHoursRemaining })}
        </Text>
      ) : null}
      {!role.hired && onHire ? (
        <Pressable
          ...
          <Text style={[styles.hireLabel, !canAfford && styles.hireLabelDisabled]}>
...
const styles = StyleSheet.create({
  ...
  contractMeta: { fontFamily: fonts.mono, fontSize: fontSizes.micro, color: colors.muted },
  hireButton: {
  ...
  hireLabelDisabled: { color: colors.muted },
});
